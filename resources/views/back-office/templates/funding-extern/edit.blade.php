...
@section('specific_js')
    <script>
        $( "#montant" ).parent().closest( '.form-group').hide()

        $('#status_cpdhSelect, input[name="status_cpdh"]').change(switchSelect);
        function switchSelect(){
            let SelVal=$( "#status_cpdhSelect" ).val();
            if (SelVal==='Accepté'){
                $( "#montant" ).parent().closest( '.form-group').show();
            }else {
                $( "#montant" ).parent().closest( '.form-group').hide()
            }
        }
